...
export function V2Workspace({providers,references,initialRun,retryRun,onRunChanged,onRetryApplied}:{providers:ApiProvider[];references:ApiReference[];initialRun?:ApiV2Run|null;retryRun?:ApiV2Run|null;onRunChanged?:(run:ApiV2Run|null)=>void;onRetryApplied?:()=>void}){
 ...
 const [run,setRunState]=useState<ApiV2Run|null>(initialRun??null),[busy,setBusy]=useState(false),[restoring,setRestoring]=useState(Boolean(initialTrack)),[error,setError]=useState(""),[shareMessage,setShareMessage]=useState(""),[restoredMessage,setRestoredMessage]=useState("");
 const timer=useRef<number|null>(null),appliedRetryId=useRef<string|null>(null);
 ...
 const poll=async(id:string)=>{try{const value=await api.v2Run(id);setRun(value);if(["queued","running"].includes(value.status))timer.current=window.setTimeout(()=>void poll(id),500);else timer.current=null;}catch(cause){setError(apiErrorMessage(cause instanceof Error?cause.message:String(cause)));timer.current=null;}};
 useEffect(()=>{const restore=(track:Track,initial=false)=>void api.v2Run(track.id).then((value)=>{setRunState(value);onRunChanged?.(value);if(["queued","running"].includes(value.status)&&timer.current==null)void poll(value.id);}).catch(()=>localStorage.removeItem(TRACK_KEY)).finally(()=>{if(initial)setRestoring(false);});const sync=(event:StorageEvent)=>{if(event.key!==TRACK_KEY)return;const track=loadTrack();if(track)restore(track);else{setRunState(null);onRunChanged?.(null);}};window.addEventListener("storage",sync);if(initialTrack)restore(initialTrack,true);else setRestoring(false);return()=>{window.removeEventListener("storage",sync);if(timer.current)window.clearTimeout(timer.current);};},[]);
