...
export const BUILTIN_UPDATE_COPY = {
  ...
  check: "检查更新",
  prepare: "下载数据到本地",
  prepareHelp: "仅把 Zenodo 数据下载到服务器本地并建立模型列表（首次需数百 MB，缓存上限 2GB，可一键清理）。下载 ≠ 更新内置：只有勾选模型后点“更新为内置参考”，数据才成为当前内置研究参考。",
  apply: "更新为内置参考",
  applyHelp: "把已下载数据中的所选模型应用为当前内置研究参考（同 ID 模型直接替换新快照，保留最近 3 个版本可回滚）。",
  applyPending: "数据已下载到本地，尚未更新为内置参考。勾选后点击“更新为内置参考”使其生效。",
  applyCurrent: "当前内置参考已使用这份已下载数据。",
  rollback: "回滚到上一版本",
  cleanCache: "清理数据集缓存",
  cancel: "取消",
  refresh: "重新检查",
  ...
  catalogReady: "模型列表已就绪，勾选后更新为内置参考",
